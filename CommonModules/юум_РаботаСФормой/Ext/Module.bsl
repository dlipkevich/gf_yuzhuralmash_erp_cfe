...
Процедура УдалитьНастройкиПоля(Настройки, ИскомоеПоле, Свойство = Неопределено) экспорт
...
Процедура СоздатьПоляКнопки(Форма, ИменаПолейКнопок, Родитель, ОбъектДляПолей = "Объект.", СобытияЭлемента = Неопределено) Экспорт
	...
	Если Элемент.Вид = ВидПоляФормы.ПолеНадписи Тогда 
		Если Элемент.Вид = ВидПоляФормы.ПолеНадписи Тогда
			Если Элемент.Гиперссылка Тогда
				...
				Элемент.Шрифт = ШрифтыСтиля.ШрифтТекста;
			КонецЕсли;
